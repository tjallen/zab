        ...
        if ($options['enabled']) {
            ...
            if (false === $uri->param('send')) {
                if ($_SERVER['REQUEST_METHOD'] == "POST") {
                    ...
                    $page->setRawContent($old_content . $twig->twig()->render($template, $data));
                }
            } else {
                switch ($uri->param('send')) {
                    case 'success':
                        $page->setRawContent($options['messages']['success']);
                    break;

                    case 'error':
                        $page->setRawContent($options['messages']['error']);
                    break;

                    case 'fail':
                        $page->setRawContent($options['messages']['fail']);
                    break;
